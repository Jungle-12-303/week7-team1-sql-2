...
- key들은 정렬되어 있다.
- 내부 노드도 실제 데이터를 가질 수 있다.
- leaf 노드도 실제 데이터를 가질 수 있다.
- 모든 leaf 노드가 같은 깊이에 오도록 균형을 유지한다.
- 노드가 가득 차면 split해서 부모로 key를 올린다.

간단히 말하면, B-Tree는 **모든 노드가 검색 key와 실제 값을 함께 가질 수 있는 다분기 균형 트리**입니다.

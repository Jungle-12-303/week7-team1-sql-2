...
하지만 B-Tree와 B+ Tree의 internal node에서 자주 쓰이는 기본 관계는 같습니다.
여기서 말하는 관계는 **현재 사용 중인 key와 child의 논리적 관계**입니다.
배열이 항상 꽉 차 있어야 한다는 뜻은 아닙니다.
...
반대로 `k+1`은 보통 **key를 k개까지 저장할 수 있으면 child 포인터는 k+1개까지 담을 수 있어야 한다**는 뜻으로 나옵니다.
즉 최대 수용 개수의 관점입니다.
항상 child를 k+1개 꽉 채워야 한다는 뜻은 아닙니다.
...
다만 node에 실제로 key가 1개만 들어 있는 순간이라면, 그 node가 사용하는 child도 2개입니다.
...
그래서 `k+1`은 문맥에 따라 아래 두 의미로 구분해서 읽어야 합니다.

```text
현재 사용 중인 key가 k개인 internal node:
실제로 연결된 child는 k+1개

최대 key 개수가 k개인 internal node:
child 포인터 공간은 k+1개까지 필요
```

### 2-4. node마다 key 개수가 달라도 되는가

node마다 들어 있는 key 개수는 달라도 됩니다.
한 node가 key를 최대 10개까지 가질 수 있다고 해서, 모든 node가 반드시 key 10개를 꽉 채워야 하는 것은 아닙니다.

예를 들어 leaf node들이 아래처럼 나뉘어 있을 수 있습니다.

```text
[1] -> [3 | 4 | 5] -> [7 | 8] -> [10 | 11 | 12 | 13 | 14]
```

각 leaf node의 key 개수는 다릅니다.
...
최대 key 개수가 10개라면, 위 node들은 모두 최대 개수를 넘지는 않았습니다.
하지만 B-Tree와 B+ Tree에는 보통 **최대 개수 조건**뿐 아니라 **최소 개수 조건**도 있습니다.
...
```text
root가 아닌 node는 최소 몇 개 이상의 key를 가져야 한다.
```

일반적인 B-Tree/B+ Tree에서는 root를 제외한 node가 대략 절반 이상 차 있어야 한다고 설명하는 경우가 많습니다.
예를 들어 최대 key 수가 10개라면, root가 아닌 node는 최소 5개 정도는 가져야 한다고 설명할 수 있습니다.

```text
최대 key 수 = 10
root가 아닌 node의 최소 key 수 ~= 5
```

그러면 아래 배치는 최대 개수만 보면 괜찮지만, 일반적인 엄격한 B-Tree/B+ Tree 규칙에서는 문제가 될 수 있습니다.

```text
[1] -> [3 | 4 | 5] -> [7 | 8] -> [10 | 11 | 12 | 13 | 14]
```

앞쪽 node들이 최소 key 수를 만족하지 못할 수 있기 때문입니다.

다만 root node는 예외입니다.
트리가 작을 때 root는 key가 1개만 있어도 괜찮을 수 있습니다.
...
```text
node마다 key 개수는 달라도 된다.
하지만 최대 key 수를 넘으면 안 된다.
root가 아닌 node는 최소 key 수도 만족해야 한다.
root는 예외적으로 key가 적어도 괜찮을 수 있다.
```

우리 프로젝트는 삭제 연산이 없는 삽입 중심의 메모리 기반 B+ Tree입니다.
그래서 삭제 때문에 node가 너무 비어버리는 상황은 크게 다루지 않습니다.
하지만 일반적인 B-Tree/B+ Tree 개념을 공부할 때는 **node마다 key 개수는 달라도 되지만, 최소~최대 범위 안에 있어야 한다**고 기억하면 됩니다.
...
k+1:
현재 key가 k개인 internal node는 child k+1개로 범위를 나누고,
최대 key가 k개라면 child 포인터 공간은 k+1개까지 필요

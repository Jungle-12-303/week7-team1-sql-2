...
단건 조회는 데이터 위치에 따라 선형 탐색 비용이 달라지는지 확인하기 위해 앞 / 중간 / 뒤 구간으로 나누어 비교합니다. 각 케이스는 `WHERE id = ?`와 `WHERE student_no = ?`를 각각 `5회` 반복 실행하여 평균과 `p95`를 기록했습니다.

| Target ID | Target Name | ID Index avg / p95 | StudentNo avg / p95 | 선형 탐색 / B+ Tree |
| ---: | --- | ---: | ---: | ---: |
| 1 | `U1` | `1076.4 / 1236 ms` | `1465.4 / 1588 ms` | `1.36x` |
| 500000 | `U500000` | `883.6 / 928 ms` | `1375.2 / 1402 ms` | `1.56x` |
| 1000000 | `U1000000` | `875.2 / 907 ms` | `1361.2 / 1377 ms` | `1.56x` |

![Point Lookup Comparison](docs/assets/point-lookup-comparison.svg)

- B+ Tree 기반 `WHERE id = ?`는 전체 구간에서 상대적으로 안정적인 응답 시간을 유지함
- `WHERE student_no = ?` 선형 탐색은 모든 케이스에서 더 오래 걸렸고, 평균 기준 `1.36x ~ 1.56x` 느렸음
- 같은 단건 조회라도 접근 경로가 인덱스인지 전체 스캔인지에 따라 체감 성능 차이가 발생함
